 ...
 */
class VDDECLSPEC VDTransform : public VDBehavior, fragcore::ITree<VDTransform> {
  private:
	VDTransform();
  ...
  public: /*	Public methods.	*/
	...
	void inline AnimationSetRotationQY(float y) {
		this->setLocalRotation(VDQuaternion(this->rotation.x(), y, this->rotation.z(), this->rotation.w()));
	}
	void inline AnimationSetRotationQZ(float z) {
		this->setLocalRotation(VDQuaternion(this->rotation.x(), this->rotation.y(), z, this->rotation.w()));
	}
	void inline AnimationSetRotationQW(float w) {
		this->setLocalRotation(VDQuaternion(this->rotation.x(), this->rotation.y(), this->rotation.z(), w));
	}

	void inline AnimationSetScaleX(float x) { this->setLocalScale(VDVector3(x, this->scale.y(), this->scale.z())); }
	void inline AnimationSetScaleY(float y) { this->setLocalScale(VDVector3(this->scale.x(), y, this->scale.z())); }
	void inline AnimationSetScaleZ(float z) { this->setLocalScale(VDVector3(this->scale.x(), this->scale.y(), z)); }
