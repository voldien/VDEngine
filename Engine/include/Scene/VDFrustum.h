...
class VDDECLSPEC VDFrustum : public VDBehavior {
  public:
	 ...
	 *
	 */
	enum FrustumPlanes { TOP = 0, BOTTOM = 1, LEFT = 2, RIGHT = 3, NEARP = 4, FARP = 5 };
	 ...
	 *
	 */
	float VDAPIENTRY getFov() const;

	/**
	 *
	 */
	float VDAPIENTRY getNear() const;

	/**
	 *
	 */
	float VDAPIENTRY getFar() const;

	/**
	 *
	 */
	float VDAPIENTRY getRatio() const;

	/**
	 *
	 */
	const VDMatrix4x4 &VDAPIENTRY getProjectionMatrix() const;

	/**
	 *	Calculate the frustum planes.
	 */
	void VDAPIENTRY calcFrustumPlanes(const VDVector3 &position, const VDVector3 &look, const VDVector3 &up,
									  const VDVector3 &right);
	 ...
	 *	@Return
	 */
	Intersection VDAPIENTRY intersectBoundingVolume(const VDBound &bounds, VDBound::BoundType type,
													const VDVector3 &position);
