...
class VDDECLSPEC VDRenderBuffer : public VDAssetObject {
  ...
  public:
	 ...
	 *
	 */
	void VDAPIENTRY setSampleStorage(unsigned int samples, unsigned int width, unsigned int height,
									 VDTexture::TextureFormat format);
	...
	inline unsigned int getRendererID() const { return this->renderer; }

  protected:
	inline void setRenderId(unsigned int render) { this->renderer = render; }
