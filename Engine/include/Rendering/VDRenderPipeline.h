...
class VDDECLSPEC VDRenderPipeLine {
  ...
  public: /*	Main rendering pipeline.	*/
	 ...
	 */
	static void VDAPIENTRY internalForwardRendering(VDFrustum *frustum, VDCamera *camera,
													VDDoubleBufferedAllocator *allocator,
													VDQueue<VDRenderer *> *renderers = nullptr);

	/**
	 *	Deferred rendering used for scene with a lot of lights.
	 */
	static void VDAPIENTRY internalDefferdRendering(VDFrustum *frustum, VDCamera *camera,
													VDDoubleBufferedAllocator *allocator,
													VDQueue<VDRenderer *> *renderers = nullptr);
