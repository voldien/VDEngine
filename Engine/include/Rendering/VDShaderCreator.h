...
class VDDECLSPEC VDShaderCreator {
  public:
	 ...
	 *
	 */
	static VDShader *createPostProcessingShader(const char *cVertexShader, const char *cFragmentShader,
												const char *cGeometryShader);
...
#ifdef _DEBUG
		return (VDAssetManager::getResourcePath(VDAssetManager::eShaderPath) + "/" + relative);
#else
		return (VDAssetManager::getResourcePath(VDAssetManager::eShaderPath) + "/" + relative + ".gz");
#endif
	}
