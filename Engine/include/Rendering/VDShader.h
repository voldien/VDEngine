...
private:
	...
	VDShader(const char* vertexPath, const char* fragmentPath,
			const char* geometryPath, const char* Name,
			const char* tessC = NULL, const char* tessE = NULL,
			unsigned int flag = 1);
public:
	~VDShader(void);
...
public:
	 ...
	 *	Release resource.
	 */
	virtual void VDAPIENTRY release(void) override;
	...
	void VDAPIENTRY release(
			unsigned int flag = (eVertex | eFragment | eGeometry | eProgram
					| eTessellationControl | eTessellationEvaulation));


	/**
	 *	bind Shader Program. All Shader Component will be enabled
	 */
	inline void bind(void){VDRenderingAPICache::bindProgram(this->getProgram());}
	 ...
	 *
	 */
	inline unsigned long int getFlagSet(void)const{
		return this->flag;
	}
	inline void setFlagSet(unsigned long int flag){
		this->flag = flag;
	}


	const struct VDShader::matrixInfo* getMatrinxInfo(void)const{
	 ...
	 *	@Return
	 */
	unsigned int VDAPIENTRY getVersion(void)const;
...
public:	/*	Set shader states.	*/
	 ...
	 *	@Return
	 */
	unsigned int VDAPIENTRY getNumSubRoutines(void)const;

	/**
	 *
	 *	@Return
	 */
	const unsigned int* getActiveRoutinesIndices(void)const;
...
public:
	 ...
	 *	@Return
	 */
	int VDAPIENTRY compileShader(const char* vertex, const char* fragment,
			const char* geomtry, const char* control, const char* evolution);
	 ...
	 *
	 */
	void VDAPIENTRY flagReference(const char* cVertexFileName,
	 ...
	 */
	void VDAPIENTRY initFlagData(void);
...
public:
	...
	std::multimap<const char*, VDShader*> VDAPIENTRY getShaderCollection(void);
...
public:	/*	Factory methods.	*/
	 ...
	 *	@Return
	 */
	static VDShader* VDAPIENTRY createShader(void);

	/**
	 *
	 *	@Return
	 */
	static VDShader* VDAPIENTRY createShader(const char* vertexPath, const char* fragmentPath, const char* geometryPath , const char* Name, const char* tessC = NULL, const char* tessE = NULL,unsigned int flag = 1);
