...
class VDDECLSPEC VDMaterial : public VDAssetObject {
  ...
  public: /*	*/
	...
	inline void VDAPIENTRY setColorv(const char *uniformName, unsigned int count, const VDColor *vector) {
		this->SetVec4v(uniformName, count, (VDVector4 *)vector);
	}
	VDColor VDAPIENTRY getColor(const char *uniformName);
	inline VDColor *VDAPIENTRY getColorv(const char *uniformName, unsigned int IndexOffset, unsigned int Count) {
		return (VDColor *)this->getVec4v(uniformName, IndexOffset, Count);
	}
	 ...
	 */
	void VDAPIFASTENTRY setOpaque();

	/**
	 *
	 *
	 */
	void VDAPIFASTENTRY setTranslucent();

  public: /*	*/
	/**
	 *	@Return
	 */
	inline bool isTranslucent() const { return this->getShader()->isFlagSet(VDShader::eShaderTranslucent); }

	/**
	 *
	 *	@Return
	 */
	inline bool isOpaque() const { return this->getShader()->isFlagSet(VDShader::eShaderOpaque); }

	/*
	 *
	 */
	inline bool isTessellation() const {
		return this->getShader()->isFlagSet(VDShader::eTessellationEvaulation | VDShader::eTessellationControl);
	}

	/*
	 *
	 */
	inline bool useStencil() const { return getShader()->isFlagSet(0x01); }

	/*
	 *
	 */
	inline bool isFrontCulling() const {
	 ...
	 *
	 */
	inline bool isBackCulling() const { return this->getShader()->isFlagSet(VDShader::eBackCulling); }

	/*
	 *
	 */
	inline bool isFrontBackCulling() const { return this->getShader()->isFlagSet(VDShader::eBothCulling); }

	/*
	 *
	 */
	void VDAPIFASTENTRY setCullingSupport(unsigned long int cullingMode) {
		// this->getShader()->setFlag( (this->getShader()->getFlagSet() & ~(VDShader::eBothCulling |
		// VDShader::eBackCulling | VDShader::eFrontCulling) ) | cullingMode );
	}
  ...
  public: /*	Static methods.	*/
	 ...
	 */
	static VDMaterial *VDAPIENTRY createMaterial();
	 ...
	 */
	static VDMaterial *VDAPIFASTENTRY getDefaultMaterial();

	/**
	 *	Get Defualt Material texture.
	 *	@Return non-null texture object.
	 */
	static VDTexture2D *VDAPIFASTENTRY getDefaultTexture();
};
