...
public:
	 ...
	 *	@Return
	 */
	int VDAPIFASTENTRY isValid(void)const;
	...
	virtual int VDAPIENTRY loadContent(const char* path,
			unsigned long int SupportFlag = IMPORT_DEFAULT);
...
private:
	 ...
	 *
	 */
	void VDAPIENTRY initNoodeRoot(const aiNode* nodes, VDGameObject* parent = NULL);
...
public:	/*	*/
	...
	VDGameObject* generateGameObjectHierarchy(void);
	VDGameObject* convertToSingle(void);
