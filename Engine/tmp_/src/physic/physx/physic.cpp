...
public:
	...
	virtual void onConstraintBreak(physx::PxConstraintInfo* constraints, PxU32 count);
	virtual void onWake(physx::PxActor** actors, PxU32 count);
	virtual void onSleep(physx::PxActor** actors, PxU32 count);
	virtual void onContact(const physx::PxContactPairHeader& pairHeader, const physx::PxContactPair* pairs, PxU32 nbPairs);
...
void VDPhysic::initialize(){
    ...
    engine.physic.gFoundation = PxCreateFoundation(PX_PHYSICS_VERSION,
	...
	engine.physic.gProfileManager = &PxProfileZoneManager::createProfileZoneManager(engine.physic.gFoundation);

	if(!engine.physic.gProfileManager){

	}

	if(!(engine.physic.gPhysic = PxCreatePhysics(PX_PHYSICS_VERSION,
	...
	engine.physic.gCooking = PxCreateCooking(PX_PHYSICS_VERSION, *engine.physic.gFoundation, PxCookingParams(PxTolerancesScale()));
	...
	engine.physic.gCudaContextManager = PxCreateCudaContextManager(*engine.physic.gFoundation, cudaDesc, engine.physic.gProfileManager);
	if(!engine.physic.gCudaContextManager){
		VDDebug::errorLog("Failed to create Nvidia Cuda Context from OpenGL Context.\n");
	}


	if(initialzeScene()){
		VDPhysic::createMaterial(VDPhysicMaterial::eDefault);
		engine.physic.gControlManager = PxCreateControllerManager(*engine.physic.gPhysxScene);
		if(engine.physic.gControlManager){
			engine.physic.gPhysiccontrollerreport = new PhysicUserControllerHitReport();
		}
		else{
...
void VDPhysic::initDebug(const char* pvd_host_ip){
	...
	PxVisualDebuggerConnectionFlags connectionFlag  = PxVisualDebuggerExt::getAllConnectionFlags();

	PvdConnection* theConnection = PxVisualDebuggerExt::createConnection(VDPhysic::getPhysicHandle()->getPvdConnectionManager(),
		pvd_host_ip, port, timeOut, connectionFlag);
	if(theConnection){
		engine.physic.gPhysic->getVisualDebugger()->setVisualizeConstraints(true);
		engine.physic.gPhysic->getVisualDebugger()->setVisualDebuggerFlag(PxVisualDebuggerFlag::eTRANSMIT_CONTACTS,true);
		engine.physic.gPhysic->getVisualDebugger()->setVisualDebuggerFlag(PxVisualDebuggerFlag::eTRANSMIT_SCENEQUERIES,true);
		engine.physic.gPhysic->getVisualDebugger()->setVisualDebuggerFlag(PxVisualDebuggerFlag::eTRANSMIT_CONSTRAINTS,true);
	}
...
int VDPhysic::initialzeScene(){
	...
	if(!sceneDesc.cpuDispatcher){
		...
		PxDefaultCpuDispatcher* mCpuDispatcher = PxDefaultCpuDispatcherCreate(engine.physic.PhysicThreadCount); // conenction with cpu.
		...
		if(mCpuDispatcher && engine.physic.gCudaContextManager){
			sceneDesc.gpuDispatcher = engine.physic.gCudaContextManager->getGpuDispatcher();
		}
	...
	engine.physic.gPhysxScene = VDPhysic::getPhysicHandle()->createScene(sceneDesc);

	if(!engine.physic.gPhysxScene){
		VDDebug::errorLog("Failed to create NVidia PhysX Scene!\n");
	}
	if(!(engine.physic.gPhysxScene->getSimulationEventCallback() == m_simulationEventcallBack)){
...
/*	m_physxScene->setClientBehaviorFlags(0,
	...
	*/
	...
	if( (VDEngine::getFlag() & VDEngine::Debug) ){	/*	TODO fix! and add VD_DEBUG CHECK	*/
		...
		VDPhysic::getPhysicScene()->setVisualizationParameter(PxVisualizationParameter::eACTOR_AXES,		1.0f);		//Enable visualization of actor's axis

		VDPhysic::getPhysicScene()->setVisualizationParameter(PxVisualizationParameter::eBODY_LIN_VELOCITY,		1.0f);		//Enable visualization of actor's axis
		VDPhysic::getPhysicScene()->setVisualizationParameter(PxVisualizationParameter::eCOLLISION_COMPOUNDS,		1.0f);		//Enable visualization of actor's axis


		VDPhysic::getPhysicScene()->setVisualizationParameter(PxVisualizationParameter::eCULL_BOX,1.0f);
		VDPhysic::getPhysicScene()->setVisualizationParameter(PxVisualizationParameter::eJOINT_LIMITS,		1.0f);
		VDPhysic::getPhysicScene()->setVisualizationParameter(PxVisualizationParameter::eJOINT_LOCAL_FRAMES,1.0f);
		VDPhysic::getPhysicScene()->setVisualizationCullingBox(PxBounds3(PxVec3(-1000),PxVec3(1000)));
	}
...
void VDPhysic::simulation(){
	engine.physic.gPhysxScene->simulate((VDTime::deltaTime() + 0.0000000001f));//(_time * 0.000001f));
...
void VDPhysic::update(){
	...
	for(x = 0; x < rigidBodyCollection.size(); x++){
		const PxTransform trans = rigidBodyCollection[x]->actor->getGlobalPose();
		rigidBodyCollection[x]->transform()->SetRigidBodyPosition(*VDCASTP(const VDVector3*,&trans.p));
		rigidBodyCollection[x]->transform()->SetRigidBodyRotation(*(const VDQuaternion*)&trans.q);
	}
...
VDPhysicMaterial* VDPhysic::createMaterial(unsigned int materialtemplate){
	if(!VDPhysic::getPhysicHandle() || !materialtemplate)
		return nullptr;
	...
	if(VDPhysic::findMaterial(materialtemplate)){//exists current
		...
		case VDPhysicMaterial::eDefault:
			material->m_material = VDPhysic::getPhysicHandle()->createMaterial(PX_MATERIAL_DEFAULT_VALUE);
			break;
		case VDPhysicMaterial::eIce:
			material->m_material = VDPhysic::getPhysicHandle()->createMaterial(PX_MATERIAL_ICE_VALUE);
			break;
		case VDPhysicMaterial::eStone:
			material->m_material = VDPhysic::getPhysicHandle()->createMaterial(PX_MATERIAL_STONE_VALUE);
			break;
		case VDPhysicMaterial::eWood:
			material->m_material = VDPhysic::getPhysicHandle()->createMaterial(PX_MATERIAL_WOOD_VALUE);
			break;
		default:
			break;
		}
...
VDPhysicMaterial* VDPhysic::findMaterial(double staticFriction, double dynamicFriction, double restitution){

	for(int x = 0; x < VDPhysicMaterial::getCollectionSize(); x++){
		// if properties match equally
		if(restitution ==  ((PxMaterial*)gPhysicMaterialCollection[x]->getMaterial())->getRestitution()
			&&
			staticFriction ==  ((PxMaterial*)gPhysicMaterialCollection[x]->getMaterial())->getStaticFriction()
			&&
			dynamicFriction ==  ((PxMaterial*)gPhysicMaterialCollection[x]->getMaterial())->getDynamicFriction()){
			return gPhysicMaterialCollection[x]; // find a material with same properties. IS that enough to consider as a same Object ??
		}
	}
	return nullptr;
}
...
PxRigidStatic* VDPhysic::createRigidStatic(VDCollider* collider){
	...
	if(collider->getActor()){
		...
		PxRigidStatic* staticActor = engine.physic.gPhysic->createRigidStatic(VDPhysic::createPxTransformInstance(collider->transform()) );
		if(staticActor)
			staticActor->userData = collider->gameObject();
		return staticActor;
	}
}
...
PxRigidDynamic* VDPhysic::createRigidDynamic(VDRigidBody* rigidBody){
	...
	if(rigidBody->getActor()){
		...
		if(rigidBody->isDynamicRigid())
			...
			rigidBody->actor = VDPhysic::convertDynamicActor(rigidBody->GetStaticActor());
			// Convert to PxRigidDynamic RigidBody
		}
	}
	else{
		PxRigidDynamic* dynamicActor = VDPhysic::getPhysicHandle()->createRigidDynamic(VDPhysic::createPxTransformInstance(rigidBody->transform()));
...
PxRigidActor* VDPhysic::copyInstanceActor(VDPhysicActor* origonalActor,VDPhysicActor* cloneActor, bool shareShape){
	PxRigidActor* cloneactor;// new actor instance
	if(origonalActor->isDynamicRigid()){ // rigidbody actor
		cloneactor = PxCloneDynamic(*VDPhysic::getPhysicHandle(),
		...
		cloneactor = PxCloneStatic(*VDPhysic::getPhysicHandle(),
			VDPhysic::createPxTransformInstance(origonalActor->transform()),
			*origonalActor->GetStaticActor());
	}
	cloneActor->actor = cloneactor;
	cloneActor->actor->userData = cloneActor->gameObject();// reference to GameObject In the Engine.
	PxShape* shape; 
	cloneActor->actor->getShapes(&shape, sizeof(PxShape*));
	if(shape)
		shape->userData = cloneActor->collider();
	return cloneactor;
}

PxCloth* VDPhysic::createClothActor(VDCloth* cloth, const HandleMeshEx* meshHandler){
	if(!cloth || !meshHandler)
		return nullptr;
	...
	cloth->actor = (PxRigidActor*)engine.physic.gPhysic->createCloth(VDPhysic::createPxTransformInstance(cloth->transform()),*fabric,vertices,PxClothFlags());
...
PxShape* VDPhysic::createShape(VDCollider* collider, unsigned int collisionFlag, HandleMeshEx* handleMesh){
	if(engine.physic.gPhysic == nullptr || collider == nullptr || collisionFlag == VDCollider::eUnknown){
		return nullptr;
	}
	...
	VDPhysicMaterial* material = VDPhysic::createMaterial(VDPhysicMaterial::eDefault);
	...
	case VDCollider::eBox:{
			...
			if(handleMesh != nullptr)	// has if
				boxGeomtry = PxBoxGeometry(*(PxVec3*)&(handleMesh->bounds.getSize()));
			if(!boxGeomtry.isValid())
				boxGeomtry.halfExtents = PxVec3(1,1,1);
			pxShape = colliderActor->createShape(boxGeomtry,
	...
	case VDCollider::eSphere:{
			...
			pxShape = colliderActor->createShape(sphereCollider,
			*(PxMaterial*)material->getMaterial(),
			VDPhysic::createPxTransformInstance(handleMesh->bounds));
		}
		break;
	case VDCollider::eTerrain:{
			PxHeightField* heightmap = VDPhysic::genHeightCollision(collider->getComponent<VDTerrain>());
			if(heightmap != nullptr){
				PxHeightFieldGeometry hfGem(heightmap, PxMeshGeometryFlags(), 1.0f,1.0f, 1.0f);
				if(!hfGem.isValid()){
					// try to fix it.
				}
				pxShape = colliderActor->createShape(hfGem, *(PxMaterial*)material->getMaterial());
			}	
		}
		break;
	case VDCollider::eMesh:{
			PxTriangleMesh* meshCollision = VDPhysic::genMeshCollision(*handleMesh); 
			if(meshCollision){
				PxTriangleMeshGeometry meshgeometry(meshCollision);
				meshgeometry.scale.scale = *(PxVec3*)&collider->transform()->getScale();
				if(lrigidbody != nullptr){
					lrigidbody->kinect(true);
					pxShape = lrigidbody->GetDynamicActor()->createShape(meshgeometry,*(PxMaterial*)material->getMaterial());//PxTransform(*(PxVec3*)&this->transform()->postion,*(PxQuat*)&this->transform()->Rotation));
					...
					pxShape = colliderActor->createShape(meshgeometry,*(PxMaterial*)material->getMaterial());//PxTransform(*(PxVec3*)&this->transform()->postion,*(PxQuat*)&this->transform()->Rotation));
				}
			}
			//meshCollision->set	
		}
	...
	case VDCollider::eConvex:{
			...
			if(convex){
				...
				convexG.scale.scale = *(PxVec3*)&collider->transform()->getScale();
				pxShape = colliderActor->createShape(convexG, *(PxMaterial*)material->getMaterial());
			}
		}
		break;
	...
	case VDCollider::eCapsule:{
		PxCapsuleGeometry capsgem(handleMesh->bounds.getSize().x() * handleMesh->bounds.getSize().z(),handleMesh->bounds.getSize().y());

		pxShape =colliderActor->createShape(capsgem,
			*(PxMaterial*)material->getMaterial(),PxTransform());
		}
		break;
...
void VDPhysic::setupFiltering(PxShape* shape, unsigned int filterGroup, unsigned int filterMask){
...
PxShape* VDPhysic::createShape(VDCollider* collider, unsigned int collisionFlag){
	...
	switch(collisionFlag){
		...
		case VDCollider::eCharacterController:
			...
			if(collider->renderer() ){
				if(collider->renderer()->getMesh() ){
					meshhandle.bounds = collider->renderer()->getMesh()->bounds.aabb;
				}
			}
		...
		case VDCollider::eConvex:{
			...
			meshhandle = collider->renderer()->getMesh()->createMeshDescription();
			shape = VDPhysic::createShape(collider, collisionFlag,
				&meshhandle);
...
PxController* VDPhysic::createCharacterController(VDTransform* transform){
	if(engine.physic.gPhysic == nullptr || transform == nullptr)
		return nullptr;

	PxCapsuleControllerDesc chDesc;
	chDesc.height = 1.0f;
	chDesc.radius = 0.5f;
	chDesc.material = (PxMaterial*)VDPhysic::createMaterial(VDPhysicMaterial::eWood)->getMaterial();
	...
	PxController* characterController = engine.physic.gControlManager->createController(chDesc);
	...
	if(shape)
		shape->userData =  transform->gameObject()->collider();
...
PxTriangleMesh* VDPhysic::genMeshCollision(const HandleMeshEx& meshHandler){
	if(!meshHandler.indicesData || !meshHandler.vertexData)
		return nullptr;

	if(meshHandler.vertexCount > (1 << 12))
		return nullptr;
	...
	if(meshHandler.vertexCount > UCHAR_MAX && meshHandler.vertexCount < UINT_MAX){
		if(meshHandler.vertexCount > USHRT_MAX)meshDesc.flags = (~PxMeshFlag::e16_BIT_INDICES & meshDesc.flags);
		meshDesc.triangles.count = (meshHandler.indicesCount / 3);
		meshDesc.triangles.stride = meshHandler.meshDescription.indicesSize() * 3;
		meshDesc.triangles.data = meshHandler.indicesData;
	}
	else{

		indicesData = callocStrideData(meshHandler.indicesData, 1, meshHandler.indicesCount, 2);
		meshDesc.triangles.count = (meshHandler.indicesCount / 3);
	...
	bool cookSucess = engine.physic.gCooking->cookTriangleMesh(meshDesc,writebuffer);
	if(!cookSucess){
		free(indicesData);
	...
	PxTriangleMesh* meshCollider = engine.physic.gPhysic->createTriangleMesh(readBuffer);
	free(indicesData);
	return meshCollider;
}
...
PxConvexMesh* VDPhysic::genConvexCollision(const HandleMeshEx& meshHandler){
	...
	if(meshHandler.vertexCount > UCHAR_MAX && meshHandler.vertexCount < USHRT_MAX){
		convexDesc.indices.count = meshHandler.indicesCount / 3;
		convexDesc.indices.data = meshHandler.indicesData;
		convexDesc.indices.stride = meshHandler.meshDescription.indicesSize() * 3;
	}
	else{// Mesh Geometry use a 8 Bits Indices Type.[PhysX don't support by it default]
		indicesData = (unsigned short*)callocStrideData(VDCASTP(void*,meshHandler.indicesData),1,meshHandler.indicesCount, 2);
		convexDesc.indices.count = meshHandler.indicesCount / 3;
	...
	PxDefaultMemoryInputData readBuffer(writebuffer.getData(), writebuffer.getSize());
	PxConvexMesh* convecMesh = engine.physic.gPhysic->createConvexMesh(readBuffer);
	free(indicesData);
	return convecMesh;
}
...
PxClothParticle* VDPhysic::genClothParticle(const HandleMeshEx& meshHandler){
	PxClothParticle* particle = (PxClothParticle*)malloc(sizeof(PxClothParticle) * meshHandler.vertexCount);

	//PxVec3 center = *CASTP(PxVec3*,&meshHandler.bounds.center);
	PxVec3 delta = 1.0f/(meshHandler.vertexCount-1) * PxVec3(15.0f, 15.0f, 15.0f);
/*	for(PxU32 i=0; i<meshHandler.vertexCount; ++i)
	{
		for(PxU32 j=0; j<meshHandler.vertexCount; ++j, ++pIt)
		{
			pIt->invWeight = j+1<meshHandler.vertexCount ? 1.0f : 0.0f;
...
PxClothFabric* VDPhysic::genClothFabric(const HandleMeshEx& meshHandler){
	...
	if(meshHandler.vertexCount > UCHAR_MAX){
		...
		Clothdesc.triangles.stride = meshHandler.meshDescription.indicesSize() * 3;
	}
	else{
		indicesData = callocStrideData(meshHandler.indicesData,
			meshHandler.meshDescription.indicesSize(),
	...
	PxClothFabric* fabric = PxClothFabricCreate(*engine.physic.gPhysic,Clothdesc, engine.physic.gPhysxScene->getGravity());
...
PxParticleSystem* VDPhysic::genParticleSystem(unsigned int maxParticleCount){
	...
	PxParticleSystem* p_particleSystem = engine.physic.gPhysic->createParticleSystem(maxParticleCount);
...
PxParticleFluid* VDPhysic::genParticleFluid( unsigned int maxParticleCount){
	...
	PxParticleFluid* p_particleFluid = engine.physic.gPhysic->createParticleFluid(maxParticleCount);
...
PxHeightFieldSample* VDPhysic::createHeightFieldSample(VDTerrain* terrain){
	PxHeightFieldSample* sample = (PxHeightFieldSample*)malloc(sizeof(PxHeightFieldSample) * (terrain->getWidth() * terrain->getHeight()));
	memset(sample,0, sizeof(PxHeightFieldSample) * (terrain->getWidth() * terrain->getHeight()));
...
PxHeightField* VDPhysic::genHeightCollision(VDTerrain* terrain){
	...
	PxHeightFieldSample* heightField = VDPhysic::createHeightFieldSample(terrain);
	PxHeightFieldDesc hfDesc;hfDesc.setToDefault();
	...
	//PxDefaultMemoryInputData readBuffer(writebuffer.getData(), writebuffer.getSize());
	PxHeightField* p_HeightField = engine.physic.gPhysic->createHeightField(hfDesc);
	free(heightField);
...
void VDPhysic::addActor(void* actor){
	...
	else engine.physic.gPhysxScene->addActor(*VDCASTP(PxActor*, actor));
}
...
PxPhysics* VDPhysic::getPhysicHandle(){
	return engine.physic.gPhysic;
}

PxScene* VDPhysic::getPhysicScene(){
...
PxTransform VDPhysic::createPxTransformInstance(const VDTransform* transform){
	if(transform)
		return PxTransform(*(const PxVec3*)&transform->getPosition(), *(const PxQuat*)&transform->getRotation());
	else
		return PxTransform();
}

PxTransform VDPhysic::createPxTransformInstance(const VDAABB& offset,const VDQuaternion& quaternion){
	return PxTransform(*(const PxVec3*)&offset.getCenter(), *(const PxQuat*)&quaternion);
}
...
void SimulationEventCallback::EventonConstraintBreak(){
	PxRigidActor* a1,*a2;VDGameObject* g1,*g2;
	for(unsigned int ncount = 0; ncount < dataBreak.count; ncount++){
		...
		g1 = VDCASTP(VDGameObject*, a1->userData); g2 = VDCASTP(VDGameObject*, a2->userData);

		for(unsigned int x = 0; x < g1->getComponentCollection().size();x++){
			if(g1->getComponentCollection()[x]->isBroodBehavior())
				VDCASTP(VDBroodBehavior*,g1->getComponentCollection()[x])->onJointBreak(0.0f);

		}
		for(unsigned int x = 0; x < g2->getComponentCollection().size();x++){
			if(g2->getComponentCollection()[x]->isBroodBehavior())
				VDCASTP(VDBroodBehavior*,g2->getComponentCollection()[x])->onJointBreak(0.0f);
...
void SimulationEventCallback::EventonContact(){
	...
	for(nCount = 0; nCount < this->dataContact.nbPair; nCount++){
		VDGameObject* g1 = VDCASTP(VDGameObject*, this->dataContact.pairs[nCount].shapes[0]->getActor()->userData);
		VDGameObject* g2 = VDCASTP(VDGameObject*,this->dataContact.pairs[nCount].shapes[1]->getActor()->userData);

		if(this->dataContact.pairs[nCount].events & PxPairFlag::eNOTIFY_TOUCH_LOST){
		...
		if(this->dataContact.pairs[nCount].events & PxPairFlag::eNOTIFY_TOUCH_FOUND){
			for(int x = 0; x < g1->getComponentCollection().size();x++){
				//if(g1->componets[x]->isBroodBehavior())
		...
		if(this->dataContact.pairs[nCount].events & PxPairFlag::eNOTIFY_TOUCH_PERSISTS){
			for(int x = 0; x < g1->getComponentCollection().size();x++){
				//if(g1->componets[x]->isBroodBehavior())
...
void SimulationEventCallback::EventonTrigger(){
	for(unsigned int nCount = 0; nCount < this->dataTrigger.count; nCount++){
		VDGameObject* g = VDCASTP(VDGameObject*,this->dataTrigger.pairs[nCount].triggerActor->userData);
		...
		case PxPairFlag::eNOTIFY_TOUCH_LOST:
			for(int x = 0; x < g->getComponentCollection().size();x++){
				if(g->getComponentCollection()[x]->isBroodBehavior())
					VDCASTP(VDBroodBehavior*,g->getComponentCollection()[x])->onTriggerExit(VDCASTP(VDCollider*,this->dataTrigger.pairs[nCount].otherShape->userData));
			}
			break;
			// On Trigger Enter 
		case PxPairFlag::eNOTIFY_TOUCH_FOUND:
			for(int x = 0; x < g->getComponentCollection().size();x++){
				if(g->getComponentCollection()[x]->isBroodBehavior())
					VDCASTP(VDBroodBehavior*,g->getComponentCollection()[x])->onTriggerEnter(VDCASTP(VDCollider*,this->dataTrigger.pairs[nCount].otherShape->userData));
		...
		case PxPairFlag::eNOTIFY_TOUCH_PERSISTS:
			for(int x = 0; x < g->getComponentCollection().size();x++){
				if(g->getComponentCollection()[x]->isBroodBehavior())
					VDCASTP(VDBroodBehavior*,g->getComponentCollection()[x])->onTriggerStay(VDCASTP(VDCollider*,this->dataTrigger.pairs[nCount].otherShape->userData));
...
void PhysicUserControllerHitReport::onShapeHit(const physx::PxControllerShapeHit& hit){
	VDGameObject* g = VDCASTP(VDGameObject*,hit.controller->getActor()->userData);
	VDCharacterControllerHit t;
	for(unsigned int x = 0; x < g->getComponentCollection().size(); x++){
		if(g->getComponentCollection()[x]->isBroodBehavior())
			VDCASTP(VDBroodBehavior*, g->getComponentCollection()[x])->onCharacterController(VDCharacterControllerHit((const physx::PxControllerShapeHit*)&hit));
	}
}

void PhysicUserControllerHitReport::onControllerHit(const PxControllersHit& hit){
	VDGameObject* g = VDCASTP(VDGameObject*,hit.controller->getActor()->userData);
	VDCharacterControllerHit t;

	for(unsigned int x = 0; x < g->getComponentCollection().size(); x++){
		if(g->getComponentCollection()[x]->isBroodBehavior())
			VDCASTP(VDBroodBehavior*, g->getComponentCollection()[x])->onCharacterController(t);
	}
}

void PhysicUserControllerHitReport::onObstacleHit(const PxControllerObstacleHit& hit){
