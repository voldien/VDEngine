...
void VDRenderingAPICache::disable(void){
	cacheEnabled = false;
}

void VDRenderingAPICache::enable(void){
	cacheEnabled = true;
}

bool VDRenderingAPICache::isEnabled(void){
	return cacheEnabled == true;
}

bool VDRenderingAPICache::isDisable(void){
	return cacheEnabled == false;
}

void VDRenderingAPICache::setClearColor(float red, float green, float blue, float alpha){
	glClearColor(red, green, blue, alpha);

	if( (clearcolor[0] != red) || (clearcolor[1] != green) || (clearcolor[2] != blue) || (clearcolor[3] != alpha) ){
...
void VDRenderingAPICache::bindTexture(unsigned int target, int texture, unsigned int index){
	if(isDisable()){
		goto setState;
	}

	/**/
	if(textureBinding.textureID[index] == texture)
		return;
...
void VDRenderingAPICache::bindTextures(unsigned int first, int count, const int* textures){
...
void VDRenderingAPICache::bindFramebuffer(unsigned int target, unsigned int framebuffer){
...
void VDRenderingAPICache::bindBufferBase(unsigned int target, unsigned int index, unsigned int buffer){
	if(isDisable())
		goto setState;
