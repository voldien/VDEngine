...
PxShape* VDPhysic::createShape(VDCollider* collider, unsigned int collisionFlag, HandleMeshEx* handleMesh){
	if(engine.physic.gPhysic == NULL || collider == NULL || collisionFlag == VDCollider::eUnknown){
		return NULL;
	}
...
PxController* VDPhysic::createCharacterController(VDTransform* transform){
	if(engine.physic.gPhysic == NULL || transform == NULL)
		return NULL;
...
PxPhysics* VDPhysic::getPhysicHandle(void){
	return engine.physic.gPhysic;
}

PxScene* VDPhysic::getPhysicScene(void){
