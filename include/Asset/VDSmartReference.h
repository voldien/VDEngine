...
public:
	...
	inline VDSmartReference(void){
		this->incre = 1;
		this->pointer = NULL;
	}

	inline VDSmartReference(const VDSmartReference& references){
		*this = references;
	}
	 ...
	 *	Override the index.
	 */
	inline void setIndexCount(unsigned int index){
		this->incre = index;
	}
	 ...
	 *	Check if equal.
	 */
	bool operator==(const VDSmartReference& reference){
		return ( this->getPointer() == reference.getPointer() );
	}
